...
def cLine(obj,x1,y1,x2,y2,d,s,tK1,tK2):  #tk3, tk4
    tags(obj,tagKS(tK1,tK2),obj.create_line(x1,y1,x2,y2,width=d,arrow=s))
def cText(obj,x,y,t,f,tK1,tK2,tK3,tK4):
    tags(obj,tagKS(tK1,tK2),obj.create_text(x,y,text=t,font=f))
...
def getKoordSystem():
    ...
    #Achsen malen
    cLine(canvas,xM,0,xM,h,2,FIRST,0,1,0,"")
    cText(canvas,xM-7,5,"y",None,0,1,1,"")
    cLine(canvas,0,yM,w,yM,2,LAST,0,0,0,"")
    cText(canvas,w-5,yM+5,"x",None,0,0,1,"")
    #x-Achsen-Markierung
    for i in range(0,w+1,50):
        cLine(canvas,x0+i,yM-3,x0+i,yM+3,1,None,0,0,2,str(stX+i))
        if stX+i!=0:
            cText(canvas,x0+i,yM+9,str(stX+i),("Arial",8),0,0,3,str(stX+i))   
        else:
            cText(canvas,x0+i+5,yM+9,str(stX+i),("Arial",8),0,0,3,str(stX+i))        
    #y-Achsen-Markierung
    for i in range(0,h+1,50):
        cLine(canvas,xM-3,y0+i,xM+3,y0+i,1,None,0,1,2,str(stY-i))
        if stY-i!=0:
            cText(canvas,xM+15,y0+i-1,str(stY-i),("Arial",8),0,1,3,str(stY-i))
...
def malePunkt(x, y, farbe, figur, name, tagArt):
    ...
    tags(canvas,[figur, name, tagArt], obj)
