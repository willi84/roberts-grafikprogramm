...
def entryVar(fr,var):
        ...
        for i in var:
            ...
            if i[7]!=_:
				for j in i[7]:
					if j[0]=="l":
							label(j[1],f[fr[0]],j[2],j[3],j[4])
					if j[0]=="b":
							button(j[1],f[fr[0]],j[2],j[3],j[4],j[5],j[6],j[7],j[8],j[9])
...
def nKoordEingeben():
    ...
    for i in range(var):
        entryVar(['NEingabe'+str(i),f['mainFrame'],_,_,_,X],[['x'+str(i),'x',LEFT,_,7,LEFT,_,_],
                                                        ['y'+str(i),'y',LEFT,_,7,LEFT,_,_]])
    objektliste(objekt)
    def setNKoordinaten():
        for i in range(var):
...
entryVar(['KoordEingabe',f['Koordinaten'],_,_,_,X],
         [['xKE',"x=",LEFT,_,4,LEFT,_,_],
          ['yKE',"y=",LEFT,_,4,LEFT,_,[["l",'leerKE',"  ", LEFT,_],
                                       ["b",'KoordEingeben',_,_,_,"eingeben",_,setKoordinaten,LEFT,_]]]])
