...
def spiegeln(farbe, art):
    ...
    for i in listeElemente:
        ...
        if i in cvWith("linie"):
            ...
            maleLinie(x1, y1, x2, y2, farbe, spiegelfigur,'P1a'+str(x1)+'/'+'b'+str(y1)+'/'+'c'+str(x2)+'/'+'d'+str(y2),"spiegellinie")
    ...
    def test2():
        ...
        if not 'KS' in idiiTags:
            ...
            if 'linie' in idiiTags:
                ...
                coo=cvCoords(idii)
                print 'x:'+str(int(coo[2]-coo[0])+2)
                print 'y:'+str(int(coo[3]-coo[1])+2)
                z1=(int(coo[3])-int(coo[1]))/(int(coo[2])-int(coo[0]))
                z2=(int(coo[2])-int(coo[0]))/(int(coo[3])-int(coo[1]))
                y1=z1*(x-v['xM'])
                x1=z2*(v['yM']-y)
                if (((v['yM']-y)>=y1-2) or ((v['yM']-y)<=y1+2)) and (((x-v['xM'])>=x1-2) or ((x-v['xM'])<=x1+2)):
                    print 'zy'; print y1
                    print 'zX'; print x1

                    print 'x'; print x-v['xM']
                    print 'y'; print v['yM']-y
                    print 'y1-2'; print y1-2
                    print 'y1+2'; print y1+2
                    print 'x1-2'; print x1-2
                    print 'x1+2'; print x1+2
                    texten= 'linie'
                else:
                    texten='Keine Linie'
...
def punkteVerbinden(event):
    ...
    if v['p1'] != []:
        if v['p2'] != []:
            maleLinie(v['p1'][0]+2,v['p1'][1]+2,v['p2'][0]+2,v['p2'][1]+2, "black", v['objekt'], 'P1a'+str(v['p1'][0])+'/'+'b'+str(v['p1'][1])+'/'+'c'+str(v['p2'][0])+'/'+'d'+str(v['p2'][1]),"linie")
