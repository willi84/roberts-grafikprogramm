...
winkel=0; p=[0,0]; k=1  # winkel: Drehwinkel, p: Spiegelungspunkt
...
def neueFigur():
    global ascii; global n; global objekt; global m
    ascii+=1
    objekt="Figur"+unichr(ascii)
    n=0
    m=0
def neu():
    global m; m=0; global n; n=0; global ascii; ascii=65;
    global objekt; objekt="Figur"+unichr(ascii)
    global winkel; winkel=0; global p; p=[0,0]; global k; k=1 
...
def getKoordSystem():
    ...
    global idArt
    #Achsen malen
    t=canvas.create_line(xM,0,xM,h,width=2, arrow=FIRST)
    canvas.addtag_withtag("KS", t)
    canvas.addtag_withtag("y-Achse", t)
    canvas.addtag_withtag("Achse", t)
    t=canvas.create_text(xM-7,5,text="y")
    canvas.addtag_withtag("KS", t)
    canvas.addtag_withtag("y-Achse", t)
    canvas.addtag_withtag("Achsenbeschriftung", t)

    t=canvas.create_line(0,yM,w,yM,width=2, arrow=LAST)
    canvas.addtag_withtag("KS", t)
    canvas.addtag_withtag("x-Achse", t)
    canvas.addtag_withtag("Achse", t)
    t=canvas.create_text(w-5,yM+5,text="x")
    canvas.addtag_withtag("KS", t)
    canvas.addtag_withtag("x-Achse", t)
    canvas.addtag_withtag("Achsenbeschriftung", t)
    ...
    if kX and kY != "":
        ...
        if weiter==True:
            ...
            if weiter==True:
                ...
                y=float(kY)
                n+=1
                malePunkt(x, y, "red", objekt, unichr(ascii)+str(n))
    if kX=="":
    ...
    if x and y != "":
        ...
        if weiter==True:
            ...
            if weiter==True:
                ...
                p=[x,y]
                n+=1
                i=canvas.create_oval(x-2+xM,-(y-2)+yM,x+2+xM,-(y+2)+yM, fill="black")
                canvas.addtag_withtag(objekt, i)
                canvas.addtag_withtag(unichr(ascii)+str(n)+"("+al+")", i)
                canvas.addtag_withtag("drehpunkt", i)
                j=canvas.create_text(x-2+xM+1,-(y-2+13)+yM,text=unichr(ascii)+str(n)+"("+al+")", font=("Arial",8,"bold"))
                canvas.addtag_withtag(objekt, j)
                canvas.addtag_withtag(unichr(ascii)+str(n)+"("+al+")", j)
                canvas.addtag_withtag("textDrehpunkt", j)
    if x=="":
...
def spiegeln(farbe, art):
    # x, y, null, p, zentr
    global objekt; global m; m+=1;
    global winkel; global p; global k
    listeElemente=[]
    ...
    for i in listeElemente:
        ...
        if i in canvas.find_withtag("text"):
            ...
            x=listeKoord[0]
            y=listeKoord[1]
            maleSpiegelPunkt(x, y, farbe, objekt, t+m*"'")
        if i in canvas.find_withtag("linie"):
            ...
            x2=xM+listeKoord2[0]
            y2=yM-listeKoord2[1]
            maleSpiegelLinie(x1, y1, x2, y2, farbe, objekt,
...
def punkteVerbinden(event):
    global p1; global p2; global objekt
    getPunkt2(event)
    if p1 != []:
        if p2 != []:
...
def malePunkt(x, y, farbe, figur, name):
    ...
    j=canvas.create_text(x-2+xM+1,-(y-2+13)+yM,text=name, font=("Arial",8,"bold"))
    canvas.addtag_withtag(figur, j)
    canvas.addtag_withtag(name, j)
    canvas.addtag_withtag("text", j)
def maleSpiegelPunkt(x, y, farbe, figur, name):
    i=canvas.create_oval(x-2+xM,-(y-2)+yM,x+2+xM,-(y+2)+yM, fill=farbe)
    canvas.addtag_withtag(figur+"Spiegelbild", i)
    canvas.addtag_withtag(name, i)
    canvas.addtag_withtag("punkt", i)
    j=canvas.create_text(x-2+xM+1,-(y-2+13)+yM,text=name, font=("Arial",8,"bold"), fill=farbe)
    canvas.addtag_withtag(figur+"Spiegelbild", j)
    canvas.addtag_withtag(name, j)
    canvas.addtag_withtag("text", j)
...
def schalten3(event):
    mb=Menubutton(wid)
    menue=Menu(canvas, tearoff=1)
    menue.add_command(label="Name")
    menue.add_command(label="Eigenschaften")
    canvas.config(menu=menue)
...
canvas.bind("<Button-3>", schalten3)
#canvas.bind("<ButtonRelease-3>", punkteVerbinden)
...
zustand1=StringVar(buttons)
zustand2=StringVar(buttons2)
def schalten1():
    if zustand1.get()=="An":
        canvas.bind("<Button-1>", punktZeichnen)
        print "Er"
...
buttonNeueFigur=Checkbutton(buttons, text="neue Figur",
                            onvalue="An", offvalue="Aus",indicatoron=0,
                            variable=zustand1,command=schalten1) #command=neueFigur
buttonNeueFigur.pack(fill=X)
buttonNeueFigur.select()
buttonFigurdaten=Checkbutton(buttons2, text="Figurdaten",
                             onvalue="An", offvalue="Aus",indicatoron=0,
                             variable=zustand2, command=schalten2)
buttonFigurdaten.pack(fill=X)
buttonFigurdaten.select()
Label(eingabefenster, text="  ").pack()
