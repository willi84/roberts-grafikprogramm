...
def setKoordinaten():
    ...
    objektliste(objekt)
def variable(fr,var):
        frame(fr[0],fr[1],fr[2],fr[3],fr[4],fr[5])
        for i in var:
            label(i[0],f[fr[0]],i[1],i[2],i[3])
            entry(i[0],f[fr[0]],i[4],i[5],i[6])
...
def nKoordEingeben():
    ...
    for i in range(var):
        variable(['NEingabe'+str(i),f['mainFrame'],_,_,_,X],[['x'+str(i),'x',LEFT,_,7,LEFT,_],
                                                        ['y'+str(i),'y',LEFT,_,7,LEFT,_]])
    objektliste(objekt)
    def setNKoordinaten():
        for i in range(var):
