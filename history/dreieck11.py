...
'''obj=cvLine(xM,0,xM,h,width=2, arrow=FIRST); tags(canvas,tagKS(1,0,""), obj)
    obj=cvText(xM-7,5,text="y"); tags(canvas,tagKS(1,1,""), obj)
    obj=cvLine(0,yM,w,yM,width=2, arrow=LAST); tags(canvas,tagKS(0,0,""), obj)
    obj=cvText(w-5,yM+5,text="x"); tags(canvas,tagKS(0,1,""), obj)
...
def cLine(obj,koord,d,s,tK1,tK2):
    tags(obj,tagKS(tK1,tK2),obj.create_line(koord,width=d,arrow=s))
...
def xyInfo(koordArt1, koordArt2, pktArt):
    listeKoordArt1=["Spiegelpunkteingabe",
                    "Koordinateneingabe"]
    listeKoordArt2=["Spiegelpunkt ",
                    "Ein Punkt ",
                 "Punkt "]
...
def tagKS(tID,tLi):
    ...
    if tLi !=None:
        for i in tLi:
            tListe.append(i)
    return tListe
'''def tagKS(l1, l2,l3,zusatz):
    global objekt
    liste1=["KS",objekt]
    liste2=["x-Achse","y-Achse"]
    liste3=["Achse","Achsenbeschriftung",
            "Markierung","Markierungsbeschriftung"]
    if zusatz != "":
        return [liste1[l1], liste2[l2], liste3[l3],zusatz]
    else:
        return [liste1[l1], liste2[l2], liste3[l3]]'''
...
def neu():
    ...
    for i in listeAlles:
        if i not in listeKS: canvas.delete(i)
        
def getKoordSystem():
    global xM, yM, x0, y0, stX, stY
    #Achsen malen
    cLine(canvas,[xM,0,xM,h],2,FIRST,[0,3,4],None)
    cText(canvas,xM-7,5,"y",None,[0,3,5],None)
    cLine(canvas,[0,yM,w,yM],2,LAST,[0,2,4],None)
    cText(canvas,w-5,yM+5,"x",None,[0,2,5],None)
    #x-Achsen-Markierung
    for i in range(0,w+1,50):
        cLine(canvas,[x0+i,yM-3,x0+i,yM+3],1,None,[0,2,6],[str(stX+i)])
        if stX+i!=0:
            cText(canvas,x0+i,yM+9,str(stX+i),("Arial",8),[0,2,7],[str(stX+i)])   
        else:
            cText(canvas,x0+i+5,yM+9,str(stX+i),("Arial",8),[0,2,7],[str(stX+i)])        
    #y-Achsen-Markierung
    for i in range(0,h+1,50):
        cLine(canvas,[xM-3,y0+i,xM+3,y0+i],1,None,[0,3,6],[str(stY-i)])
        if stY-i!=0:
            cText(canvas,xM+15,y0+i-1,str(stY-i),("Arial",8),[0,3,7],[str(stY-i)])
...
def nKoordEingeben():
    ...
    '''n1=n2=n3=n4=n5=Frame()
    n1_XEntry=n1_YEntry=n2_XEntry=n2_YEntry=n3_XEntry=n3_YEntry=n4_XEntry=n4_YEntry=n5_XEntry=n5_YEntry=Entry()'''
    def nKE(master,frame,xE,xY):
        frame=Frame(master)
        frame.pack(fill=X)
        Label(frame,text="x=").pack(side=LEFT)
        xE=Entry(frame,width=7)
        xE.pack(side=LEFT)
        Label(frame,text="y=").pack(side=LEFT)
        yE=Entry(frame,width=7)
        yE.pack(side=LEFT)
    nKE(nKoordRoot,n1,n1_XEntry,n1_YEntry)
    nKE(nKoordRoot,n2,n2_XEntry,n2_YEntry)
    nKE(nKoordRoot,n3,n3_XEntry,n3_YEntry)
    nKE(nKoordRoot,n4,n4_XEntry,n4_YEntry)
    nKE(nKoordRoot,n5,n5_XEntry,n5_YEntry)
        ...
        if kX and kY != "":
            ...
            if weiter==True:
                ...
                if weiter==True:
                    ...
                    n+=1
                    malePunkt(x, y, "red", objekt, str(unichr(ascii))+str(n),"punkt")
        if kX=="" and kY !="": xyInfo(1,1,"x")
        if kY=="" and kX !="": xyInfo(1,1,"y")
    objektliste(objekt)
    def setNKoordinaten():
    ...
    if x and y != "":
        ...
        if weiter==True:
            ...
            if weiter==True:
                ...
                p=[x,y]
                n+=1
                cOval(canvas,x-2+xM,-(y-2)+yM,x+2+xM,-(y+2)+yM,"black",[1,10],
                      [str(unichr(ascii))+str(n)+"("+al+")"])
                cText(canvas,x-2+xM+1,-(y-2+13)+yM,str(unichr(ascii))+str(n)+"("+al+")",
...
def malePunkt(x, y, farbe, figur, name, tagArt):
    cOval(canvas,x-2+xM,-(y-2)+yM,x+2+xM,-(y+2)+yM,farbe,None,[figur, name, tagArt])
    cText(canvas,x-2+xM+1,-(y-2+13)+yM,name,("Arial",8,"bold"),[14],[figur, name])
    objektliste(figur)

def maleLinie(x1, y1, x2, y2, farbe, figur, name, tagArt):
    koord=linieAnpassen(x1, y1, x2, y2)
    cLine(canvas,koord,farbe,None,[figur, name, tagArt])
    objektliste(figur)
...
def cbPunktZeichnen():
    ...
    if zustand1==True:
        ...
        canvas.bind("<Button-1>", leer)
def cbPunkteVerbinden():
    global zustand2
    ...
    if zustand2==True:
        ...
        canvas.bind("<Button-1>", leer)
        canvas.bind("<ButtonRelease-1>", leer)
def leer(): pass
...
buttonNeueFigur=Button(buttons, image=malen_in,bg="black",
                            relief=FLAT,command=cbPunktZeichnen) #command=neueFigur
buttonNeueFigur.pack(side=LEFT)
buttonFigurdaten=Button(buttons, image=linieVerbinden_in, relief=FLAT, bg="black", command=cbPunkteVerbinden)
