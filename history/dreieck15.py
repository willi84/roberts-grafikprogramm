...
def aktion(obj,evt):
    for i in evt:
        obj.bind(i[0],i[1])
def nKE(fr,num):
        frame('NEingabe'+num,fr,_,_,_,X)
        label('x'+num,f['NEingabe'+num],'x',LEFT,_)
        entry('x'+num,f['NEingabe'+num],7,LEFT,_)
        label('y'+num,f['NEingabe'+num],'y',LEFT,_)
        entry('y'+num,f['NEingabe'+num],7,LEFT,_)
...
def getKoordSystem():
    ...
    #Achsen malen
    cLine(cv,[xM,0,xM,h],2,FIRST,_,[0,3,4],_)
    cText(cv,xM-7,5,"y",_,[0,3,5],_)
    cLine(cv,[0,yM,w,yM],2,LAST,_,[0,2,4],_)
    cText(cv,w-5,yM+5,"x",_,[0,2,5],_)
    #x-Achsen-Markierung
    for i in range(0,w+1,50):
        cLine(cv,[x0+i,yM-3,x0+i,yM+3],1,_,_,[0,2,6],[str(stX+i)])
        if stX+i!=0:
    ...
    for i in range(0,h+1,50):
        cLine(cv,[xM-3,y0+i,xM+3,y0+i],1,_,_,[0,3,6],[str(stY-i)])
        if stY-i!=0:
...
def nKoordEingeben():
    ...
    '''def nKE(fr,num):
        frame('NEingabe'+str(i),f['mainFrame'],_,_,_,X)
        label('x'+num,f['NEingabe'+num],'x',LEFT,_)
        entry('x'+num,f['NEingabe'+num],7,LEFT,_)
        label('y'+num,f['NEingabe'+num],'y',LEFT,_)
        entry('y'+num,f['NEingabe'+num],7,LEFT,_)'''
    ...
    if x and y != "":
        ...
        if weiter==True:
            ...
            if weiter==True:
                ...
                p=[x,y]
                n+=1
                cOval(cv,x-2+xM,-(y-2)+yM,x+2+xM,-(y+2)+yM,"black",[1,10],[str(unichr(ascii))+str(n)+"("+al+")"])
                cText(cv,x-2+xM+1,-(y-2+13)+yM,str(unichr(ascii))+str(n)+"("+al+")",("Arial",8,"bold"),[1,11],[str(unichr(ascii))+str(n)+"("+al+")"])                            
    if x=="": xyInfo(0,0,"x")
    if y=="": xyInfo(0,0,"y")
...
def malePunkt(x, y, farbe, figur, name, tagArt):
    cOval(cv,x-2+xM,-(y-2)+yM,x+2+xM,-(y+2)+yM,farbe,_,[figur, name, tagArt])
...
def maleLinie(x1, y1, x2, y2, farbe, figur, name, tagArt):
    ...
    cLine(cv,koord,_,_,farbe,_,[figur, name, tagArt])  
    objektliste(figur)
...
#----
cvWith=cv.find_withtag; cvAll=cv.find_all; cvClose=cv.find_closest; cvCoords=cv.coords
getKoordSystem()
...
frame('KoordEingabe', f['Koordinaten'],_,_,_,X)
label('xKE',f['KoordEingabe'],"x=", LEFT, _)
entry('xKE',f['KoordEingabe'],4,LEFT,_)
label('yKE',f['KoordEingabe'],"y=", LEFT, _)
entry('yKE',f['KoordEingabe'],4,LEFT,_)
label('leerKE',f['KoordEingabe'],"  ", LEFT, _)
button('KoordEingeben',f['KoordEingabe'],_,_,_,"eingeben",_,setKoordinaten,LEFT,_)
...
label('SPE',f['SPE'],"y=",LEFT,_)
entry('ySP',f['SPE'],4,LEFT,_)
label('leerSPE',f['SPE'],"  ",LEFT,_)
