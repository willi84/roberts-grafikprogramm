...
def getKoordSystem():
    ...
    #Achsen malen
    cLine(canvas,[xM,0,xM,h],2,FIRST,None,[0,3,4],None)
    cText(canvas,xM-7,5,"y",None,[0,3,5],None)
    cLine(canvas,[0,yM,w,yM],2,LAST,None,[0,2,4],None)
    cText(canvas,w-5,yM+5,"x",None,[0,2,5],None)
    #x-Achsen-Markierung
    for i in range(0,w+1,50):
        cLine(canvas,[x0+i,yM-3,x0+i,yM+3],1,None,None,[0,2,6],[str(stX+i)])
        if stX+i!=0:
    ...
    for i in range(0,h+1,50):
        cLine(canvas,[xM-3,y0+i,xM+3,y0+i],1,None,None,[0,3,6],[str(stY-i)])
        if stY-i!=0:
...
def nKoordEingeben():
    ...
    hptframe=Frame(nKoordRoot)
    hptframe.pack()
    f=[]; x=[]; y=[]
    def nKE():
        f.append(Frame(hptframe))
    ...
    def setNKoordinaten():
        ...
        nKoordRoot.destroy()
    buttonNKoord=Button(hptframe, text="eingeben", command=setNKoordinaten)
    buttonNKoord.pack()
    nKoordRoot.mainloop()
...
def maleLinie(x1, y1, x2, y2, farbe, figur, name, tagArt):
    ...
    cLine(canvas,koord,None,None,farbe,None,[figur, name, tagArt])  
    objektliste(figur)
