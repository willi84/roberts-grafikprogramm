...
def infobox():
    ...
    if v['idObj']!=_:
        ...
        for i in cvTags(v['idObj']):
            ...
            if i[0]=="!":
                ...
                if i[1]=="p":
                    for j in cvTags(cvWith('@'+liste[1])):
                        if j[0]=="%":
                            info['p']=j[1:]
                        if j[0]=="[":
    ...
    if info!={}:
        ...
        if info['art'] in ['Spiegelpunkt','Spiegellinie']:
            ...
            spiegelung['p']=('Spiegelung '+spArt[info['art']]+'\am Punkt '+info['p']+" ("+
...
def nKoordEingeben():
    ...
    window('nKoordRoot',"Eingabe mehrerer Punkte",_)
            ...
            while not beenden: 
                for i in v['aktuellesObjekt']:
                    if "'" not in i:
                        ...
                        beenden=True
def setNamePunkt():
    window('changePoint',"Name eines Punktes ändern",_)
...
#----1. Start des Masters ----------------------------------------------------------
window('Hauptfenster',"Roberts Grafikprogramm 4.0",'fertiges programm/head.ico')
v['thread2']=f['Hauptfenster'].after(2000,showInfo)
#-------
